...
- `/app/api/chat/route.ts` — POST handler using `streamText` with 3 tools
- `/lib/kb.ts` — loads markdown from `/kb`, chunks by paragraph, embeds with `text-embedding-3-small`, cosine similarity search
- `/lib/tools.ts` — tool definitions if extracted from route (optional)
- `/lib/seed-requests.ts` — 10 synthetic demo requests
...
- Use `ToolLoopAgent` class — use `streamText` with tools directly (simpler for this scope)
