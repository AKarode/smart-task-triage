...
├── lib/
│   ├── tools.ts                      # Tool definitions (search_kb, classify, draft)
...
### `/app/api/chat/route.ts` — The Core Route
...
Be concise and actionable.`,
    ...
    tools: {
      search_kb: {
        description: 'Search internal knowledge base for relevant policies, SOPs, and guidelines. Returns the top matching document chunks with similarity scores.',
      ...
      classify_request: {
        description: 'Classify a request into a category, priority level, and recommended team queue. Use this after understanding the request.',
        inputSchema: z.object({
        ...
        execute: async (input) => {
          // This tool is auto-executed — just returns the classification
      ...
      draft_response: {
        ...
        // The absence of execute + needsApproval pattern means the SDK pauses here
      },
    },
  });
...
Split-pane layout: left sidebar (request list), center (chat interface), right (trace panel).
...
// Key patterns:
// - messages.parts array contains tool invocation parts
// - Check part.type === 'tool-draft_response' for the approval UI
// - Use addToolOutput({ tool: 'draft_response', toolCallId, output }) to approve
// - Trace data comes from tool parts with state transitions
```
...
{message.parts?.map((part, i) => {
  // Tool invocation that needs approval
...
## README Talking Points (for the interview)

Include these in the README under a "Design Decisions" section:
1. "Used AI SDK 6's native tool calling with the needsApproval pattern for human-in-the-loop, avoiding custom approval infrastructure"
2. "Embedded KB vectors in-memory using AI SDK's embed function — eliminates vector DB dependency while demonstrating the RAG pattern. Production version would use pgvector or similar"
3. "Chose cosine similarity over a hosted search because for <100 chunks the latency is negligible and it removes an infrastructure dependency"
4. "Provider-agnostic: switching from OpenAI to Anthropic requires changing one environment variable"
5. "Trace visibility is a first-class feature, not an afterthought — every tool call, its inputs, outputs, and timing are visible to the operator"
